...
public class GFXSurface extends Activity implements OnTouchListener{
	...
	plusC = BitmapFactory.decodeResource(getResources(), R.drawable.c);
	plusA = BitmapFactory.decodeResource(getResources(), R.drawable.a);
    ...
    public class MyBringBackSurface extends SurfaceView implements Runnable{
    	...
    	while(isRunning){
    	    ...
    	    if(sX != 0 && sY != 0){
    		canvas.drawBitmap(plusA, sX-(plusA.getWidth()/2), (sY-plusA.getHeight()/2), null);
    	    }
    	    if(fX != 0 && fY != 0){
    		canvas.drawBitmap(test, fX-(test.getWidth()/2)-aniX, fY-(test.getHeight()/2)-aniY, null);
    		canvas.drawBitmap(plusC, fX-(plusC.getWidth()/2), (fY-plusC.getHeight()/2), null);
    	    }
    	    aniX = aniX + scaledX;
    	    aniY = aniY + scaledY;
